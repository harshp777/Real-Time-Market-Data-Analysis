...
TYPE = EXTERNAL_STAGE
STORAGE_PROVIDER = S3
STORAGE_AWS_ROLE_ARN = ''--> from aws role
ENABLED = TRUE
STORAGE_ALLOWED_LOCATIONS = (''); --> destination bucket/folder 


CREATE OR REPLACE STAGE new_s3_stage
    URL = '' --> destination bucket/folder
    STORAGE_INTEGRATION = S3_INTEGRATION_PRO;
    -- Create a file format (Modify according to your file structure)
...
-- Create Snowpipe to load data into the table
CREATE OR REPLACE PIPE my_new_snowpipe
AUTO_INGEST = TRUE
AS
